...
import { OfferSchema, AcceptOfferSchema } from '../schemas';
...
router.post('/list', async (req: AuthRequest, res: Response) => {
  try {
    ...
    throw new ApiError(500, 'Failed to list drivers');
  }
});

// Toggle driver availability
router.post('/toggle/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;

    if (req.userId !== id) {
      throw new ApiError(403, 'Can only toggle your own availability');
    }
...
router.post('/:rideId/offer', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (ride.negotiationExpiresAt && ride.negotiationExpiresAt < new Date()) {
      throw new ApiError(400, 'Negotiation window has expired');
    }
    ...
    if (existingOffer) {
      throw new ApiError(400, 'You already have an offer for this ride');
    }
    ...
    throw new ApiError(500, 'Failed to submit offer');
  }
});

// Accept an offer (rider action)
router.post('/offers/:offerId/accept', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (offer.ride.riderId !== req.userId) {
      throw new ApiError(403, 'Only the ride requester can accept offers');
    }
    ...
    throw new ApiError(500, 'Failed to accept offer');
  }
});

// Get offers for a ride
router.get('/:rideId/offers', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { rideId } = req.params;
    ...
    if (ride.riderId !== req.userId) {
      throw new ApiError(403, 'Only the ride requester can view offers');
    }
    ...
    throw new ApiError(500, 'Failed to get offers');
  }
});

// Get driver statistics
router.get('/stats/:driverId', async (req: AuthRequest, res: Response) => {
  try {
    ...
    throw new ApiError(500, 'Failed to get driver statistics');
  }
});
